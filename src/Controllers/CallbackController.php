...
class CallbackController extends Controller
{
     ...
     */
    protected $aryChargebacks = ['PRZELEWY24_REFUND', 'RETURN_DEBIT_SEPA', 'REVERSAL', 'CREDITCARD_BOOKBACK', 'CREDITCARD_CHARGEBACK', 'PAYPAL_BOOKBACK', 'REFUND_BY_BANK_TRANSFER_EU', 'CASHPAYMENT_REFUND', 'GUARANTEED_INVOICE_BOOKBACK', 'GUARANTEED_SEPA_BOOKBACK', 'POSTFINANCE_REFUND', 'APPLEPAY_BOOKBACK', 'APPLEPAY_CHARGEBACK'];
    ...
    public function processCallback()
    {
        ...
        $orderLanguage= $this->orderLanguage($orderob);
            ...
            {
                $nnTransactionHistory->additionalInfo = ['type' => 'credit', 'tid_status' => $this->aryCaptureParams['tid_status'], 'product_id' => $this->aryCaptureParams['product_id'], 'plugin_version' => NovalnetConstants::PLUGIN_VERSION];
                ...
                {
                        if ($nnTransactionHistory->order_paid_amount < $nnTransactionHistory->order_total_amount || ($this->aryCaptureParams['payment_type'] == 'ONLINE_TRANSFER_CREDIT' && $createPaymentEntry && ($nnTransactionHistory->order_paid_amount < $nnTransactionHistory->order_total_amount) ))
                        {
